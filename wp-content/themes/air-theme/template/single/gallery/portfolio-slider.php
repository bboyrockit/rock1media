...
if($portfolio){ ?>
    ...
    <div class="blog-unit-gallery-wrap single-fullwidth-slider-wrap">
        ...
        <?php } else { ?>
        <div class="owl-carousel" data-item="1" data-center="true" data-margin="30" data-autowidth="true" data-slideby="1" data-showdot="false" data-nav="false" data-loop="false" data-lazy="false">    
        <?php } ?>
            <?php foreach($portfolio as $image){ 
                $thumb_width = 1000;
                $thumb_height = ''; 
                $thumb = wp_get_attachment_image($image, 'all');
                $thumb_src = wp_get_attachment_image_src($image, 'all'); 
                if($thumb_src){
            ...
            <section class="item">
                    <div class="carousel-img-wrap">
                        <img src="<?php echo esc_url($thumb_url); ?>" width="<?php echo esc_attr($thumb_width); ?>" height="<?php echo esc_attr($thumb_height); ?>" alt="<?php echo get_the_title($image); ?>" class="single-fullwidth-slider-carousel-img attachment-all size-all"> 
                    </div>
                </section>
            <?php } ?>
         </div> 
